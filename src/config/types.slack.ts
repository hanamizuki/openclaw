...
export type SlackChannelConfig = {
   ...
   * Only takes effect in channel/group/MPIM contexts (not DMs). Requires that
   * the bot's user id is resolvable (e.g. via `auth.test`) so the bot/non-bot
   * mention split can be detected reliably. Default: false.
   */
